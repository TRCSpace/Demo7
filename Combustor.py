...
class Combustor(Component):
    def __init__(self, nodes):
        ...
        self.T_r = nodes.T_r[0]
        self.OFR = nodes.OFR[0]
        self.P_c = nodes.P_c[0]
        self.mdot = nodes.mdot_t[0]
        ...
        self.mdot_f = self.mdot_t * (1 / (self.OFR + 1))
        # Propellant Density:
        self.rho_f = cp.CoolProp.PropsSI('D', 'P', self.P_c, 'T', self.T_r, nodes.fuel)
        # Propellant viscosity:
        self.mu_f = cp.CoolProp.PropsSI('viscosity', 'P', self.P_c, 'T', self.T_r, nodes.fuel)
        # Propellant Surface tension:
        self.sigma_f = cp.CoolProp.PropsSI('surface_tension', 'P', self.P_c, 'T', self.T_r, nodes.fuel)
